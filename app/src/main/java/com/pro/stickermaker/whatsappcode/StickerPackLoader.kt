...
import com.pro.stickermakerimpl.whatsappcode.StickerContentProvider.Companion.AUTHORITY_URI
import com.pro.stickermaker.whatsappcode.StickerPackValidator.verifyStickerPackValidity
...
import com.pro.stickermakerimpl.whatsappcode.StickerContentProvider
...
internal object StickerPackLoader {
    ...
    fun fetchStickerPacks(context: Context): ArrayList<StickerPack> {
        val cursor = context.contentResolver.query(AUTHORITY_URI, null, null, null, null)
        ...
        for (stickerPack in stickerPackList) {
            ...
            stickerPack.set_Stickers(stickers)
        ...
        for (sticker in stickers) {
            ...
            try {
                bytes = fetchStickerAsset(
                    Objects.requireNonNull(stickerPack.identifier).toString(),
                    Objects.requireNonNull(sticker.image_file).toString(),
                    context.contentResolver
                )
                check(bytes.size > 0) { "Asset file is empty, pack: " + stickerPack.name + ", sticker: " + sticker.image_file }
                sticker.size = bytes.size.toLong()
            } catch (e: IOException) {
                throw IllegalStateException(
                    "Asset file doesn't exist. pack: " + stickerPack.name + ", sticker: " + sticker.image_file,
                    e
                )
    ...
    private fun fetchFromContentProvider(cursor: Cursor): ArrayList<StickerPack> {
        ...
        do {
            ...
            val licenseAgreementWebsite = cursor.getString(
                cursor.getColumnIndexOrThrow(
                    StickerContentProvider.LICENSE_AGREEMENT_WEBSITE
                )
            )
            ...
            stickerPack.android_play_store_link = androidPlayStoreLink
            stickerPack.ios_app_store_link = iosAppLink
