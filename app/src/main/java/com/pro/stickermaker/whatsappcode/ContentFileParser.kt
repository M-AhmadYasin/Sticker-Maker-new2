...
import com.pro.stickermaker.sahredpref.SharedPrefs
...
internal object ContentFileParser {
    ...
    fun setContext(ctx: Context): ContentFileParser {
        ContentFileParser.ctx = ctx
        return this
    }
    ...
    private fun readStickerPacks(reader: JsonReader): List<StickerPack> {
        ...
        for (stickerPack in stickerPackList) {
            ...
            stickerPack.ios_app_store_link = iosAppStoreLink
        }
        */
        val stickerPackList = ctx?.let { SharedPrefs.readStickerJSONFromPrefs(it) }?: arrayListOf()
        return stickerPackList.filterNotNull()
    }
    ...
    private fun readStickerPack(reader: JsonReader): StickerPack {
        ...
        check(!(stickerList == null || stickerList.isEmpty())) { "sticker list is empty" }
