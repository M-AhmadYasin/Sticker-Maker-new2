...
internal object StickerPackValidator {
    ...
    fun verifyStickerPackValidity(context: Context, stickerPack: StickerPack) {
        ...
        check(!TextUtils.isEmpty(stickerPack.tray_image_file)) { "sticker pack tray id is empty, sticker pack identifier:" + stickerPack.identifier }
        check(
            !(!TextUtils.isEmpty(stickerPack.android_play_store_link) && !isValidWebsiteUrl(
                stickerPack.android_play_store_link
            ))
        ) { "Make sure to include http or https in url links, android play store link is not a valid url: " + stickerPack.android_play_store_link }
        check(
            !(!TextUtils.isEmpty(stickerPack.android_play_store_link) && !isURLInCorrectDomain(
                stickerPack.android_play_store_link,
                PLAY_STORE_DOMAIN
            ))
        ) { "android play store link should use play store domain: $PLAY_STORE_DOMAIN" }
        check(!(!TextUtils.isEmpty(stickerPack.ios_app_store_link) && !isValidWebsiteUrl(stickerPack.ios_app_store_link))) { "Make sure to include http or https in url links, ios app store link is not a valid url: " + stickerPack.ios_app_store_link }
        check(
            !(!TextUtils.isEmpty(stickerPack.ios_app_store_link) && !isURLInCorrectDomain(
                stickerPack.ios_app_store_link,
                APPLE_STORE_DOMAIN
            ))
        ) { "iOS app store link should use app store domain: $APPLE_STORE_DOMAIN" }
        check(
            !(!TextUtils.isEmpty(stickerPack.license_agreement_website) && !isValidWebsiteUrl(
                stickerPack.license_agreement_website
            ))
        ) { "Make sure to include http or https in url links, license agreement link is not a valid url: " + stickerPack.license_agreement_website }
        check(
            !(!TextUtils.isEmpty(stickerPack.privacy_policy_website) && !isValidWebsiteUrl(
                stickerPack.privacy_policy_website
            ))
        ) { "Make sure to include http or https in url links, privacy policy link is not a valid url: " + stickerPack.privacy_policy_website }
        check(!(!TextUtils.isEmpty(stickerPack.publisher_website) && !isValidWebsiteUrl(stickerPack.publisher_website))) { "Make sure to include http or https in url links, publisher website link is not a valid url: " + stickerPack.publisher_website }


        check(
            !(!TextUtils.isEmpty(stickerPack.publisher_email) && !Patterns.EMAIL_ADDRESS.matcher(
                stickerPack.publisher_email.toString()
            ).matches())
        ) { "publisher email does not seem valid, email is: " + stickerPack.publisher_email }


        try {
            val stickerAssetBytes = StickerPackLoader.fetchStickerAsset(
                stickerPack.identifier!!,
                stickerPack.tray_image_file!!,
                context.contentResolver
            )
            check(stickerAssetBytes.size <= TRAY_IMAGE_FILE_SIZE_MAX_KB * KB_IN_BYTES) { "tray image should be less than " + TRAY_IMAGE_FILE_SIZE_MAX_KB + " KB, tray image file: " + stickerPack.tray_image_file }
            val bitmap = BitmapFactory.decodeByteArray(stickerAssetBytes, 0, stickerAssetBytes.size)
            check(!(bitmap.height > TRAY_IMAGE_DIMENSION_MAX || bitmap.height < TRAY_IMAGE_DIMENSION_MIN)) { "tray image height should between " + TRAY_IMAGE_DIMENSION_MIN + " and " + TRAY_IMAGE_DIMENSION_MAX + " pixels, current tray image height is " + bitmap.height + ", tray image file: " + stickerPack.tray_image_file }
            check(!(bitmap.width > TRAY_IMAGE_DIMENSION_MAX || bitmap.width < TRAY_IMAGE_DIMENSION_MIN)) { "tray image width should be between " + TRAY_IMAGE_DIMENSION_MIN + " and " + TRAY_IMAGE_DIMENSION_MAX + " pixels, current tray image width is " + bitmap.width + ", tray image file: " + stickerPack.tray_image_file }
        } catch (e: IOException) {
        ...
        for (sticker in stickers) {
            validateSticker(
                ...
                stickerPack.identifier!!,
                sticker,
                stickerPack.animated_sticker_pack
            )
        }
    }
    ...
    private fun validateSticker(
        ...
        check(sticker.emojis!!.size <= EMOJI_MAX_LIMIT) { "emoji count exceed limit, sticker pack identifier: " + identifier + ", filename: " + sticker.image_file }
        check(sticker.emojis!!.size >= EMOJI_MIN_LIMIT) { "To provide best user experience, please associate at least 1 emoji to this sticker, sticker pack identifier: " + identifier + ", filename: " + sticker.image_file }
        check(!TextUtils.isEmpty(sticker.image_file)) { "no file path for sticker, sticker pack identifier:$identifier" }
        validateStickerFile(context, identifier, sticker.image_file!!, animatedStickerPack)
    }
    ...
    private fun validateStickerFile(
        ...
        try {
            val stickerInBytes =
                StickerPackLoader.fetchStickerAsset(identifier, fileName, context.contentResolver)
    ...
    private fun checkStringValidity(string: String) {
        ...
        check(string.matches(pattern.toRegex())) { "$string contains invalid characters, allowed characters are a to z, A to Z, _ , ' - . and space character" }
